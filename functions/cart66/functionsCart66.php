...
function getStatus( $status ) {
  global $wpdb; $table = $wpdb->prefix . "cart66_cart_settings";
...
function getInformation( $row, $field) {
  global $wpdb;
  $result = $wpdb->get_row("SELECT * FROM $wpdb->postmeta WHERE post_id = " . $row['ID'] . " and meta_key = '" . $field ."'", ARRAY_A);
...
function add_comment( $comment, $id ) {
  ...
  $result = $wpdb->update( $table,  array(  'notes' => $row['notes'] . '&#10;' . $comment ),  array( 	'id' => $id ) );
