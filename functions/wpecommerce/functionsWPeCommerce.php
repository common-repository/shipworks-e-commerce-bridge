...
function getWeight( $row ) {
  global $wpdb;
  $table = $wpdb->postmeta;	$row = $wpdb->get_row("SELECT * FROM " . $table . " WHERE post_id = " . $row['prodid'] . " and meta_key = '_wpsc_product_metadata'" , ARRAY_A);
...
function getVariationValue( $attributeid ) {
  global $wpdb;
  $table = $wpdb->term_taxonomy;	$row = $wpdb->get_row("SELECT * FROM " . $table . " WHERE term_taxonomy_id = " . $attributeid, ARRAY_A);
  $table = $wpdb->terms;
...
function getVariation( $attributeid ) {
  global $wpdb;
  $table = $wpdb->term_taxonomy;	$row = $wpdb->get_row("SELECT * FROM " . $table . " WHERE term_taxonomy_id = " . $attributeid , ARRAY_A);
  $termid = $row['parent'];	$table = $wpdb->terms;	$row = $wpdb->get_row("SELECT * FROM " . $table . " WHERE term_id = " . $termid , ARRAY_A);
  return $row['name'];
}
